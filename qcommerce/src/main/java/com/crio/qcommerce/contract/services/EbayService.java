...
public class EbayService implements CsvParser, SaleAnalytics {
  private File file;

  protected EbayService(File file) {
    this.file = file;
  }
  ...
  public SaleAggregate getSaleAggregateByYear(int year) throws AnalyticsException {
    ...
    for (SalesData saleData : salesData) {
      LocalDate date = LocalDate.parse(saleData.getDate(), DateTimeFormatter.ofPattern("MM/dd/uuuu"));
      String transactionStatus = saleData.getTransactionStatus();

      if ((transactionStatus.equalsIgnoreCase("delivered") || transactionStatus.equalsIgnoreCase("complete"))
          && date.getYear() == year) {
